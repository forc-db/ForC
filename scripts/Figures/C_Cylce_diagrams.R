...
C_cycle_closure <- read.csv("numbers_and_facts/C_cycle_closure.csv", stringsAsFactors = F)
...
variables <- list(NEP = list(variable.type = "flux", #####
                  ...
                  R_root = list(variable.type = "flux",
                                variable.name  = "R[root]",
                                coordinates = list(x0 = 5.8, y0 = 2.1, x1 = 5.5, y1 = 8.8),
                                y.adjust = 1,
                                x.adjust = 0,
                                move.text.box = "null",
                                need.semi.transparent.box = TRUE),
                  R_het_soil = list(variable.type = "flux",
                                    variable.name  = "R[het-soil]",
                  ...
                  biomass_root_coarse = list(variable.type = "stock",
                                             variable.name  = "coarse~root~biomass",
                  ...
                  biomass_root_fine = list(variable.type = "stock",
                                           variable.name  = "fine~root~biomass",
                  ...
                  deadwood_standing = list(variable.type = "stock",
                                           variable.name  = "standing~dead~wood",
                  ...
                  biomass_ag_woody = list(variable.type = "stock",
                                          variable.name  = "woody~ag~biomass",
                                          coordinates = list(x0 = 10.25, y0 = 6, x1 = 12.55, y1 = 6.8),
                                          y.adjust = 0,
                                          x.adjust = 0,
                                          move.text.box = "null",
                                          need.semi.transparent.box = FALSE),
                  biomass_foliage = list(variable.type = "stock",
                                         variable.name  = "foliage~biomass",
                                         coordinates = list(x0 = 10.25, y0 = 8.85, x1 = 12.55, y1 = 9.65),
                                         y.adjust = 0,
                                         x.adjust = 0,
                                         move.text.box = "null",
                                         need.semi.transparent.box = FALSE),
                  biomass_ag = list(variable.type = "stock",
                                    variable.name  = "aboveground~biomass",
                                    coordinates = list(x0 = 11.75, y0 = 7.4, x1 = 14.5, y1 = 8.2),
                                    y.adjust = 0,
                                    x.adjust = 0,
                                    move.text.box = "null",
                                    need.semi.transparent.box = FALSE)
...
for(b in unique(ForC_biome_averages$Biome)){
  ...
  for(v in names(variables)) {
    ...
    failure_C_closure <- grepl("MATURE", b)  & v %in% C_cycle_closure$variable.diagram[C_cycle_closure$Biome %in% b & C_cycle_closure$lack.of.closure %in% 1]
    ...
    if(variable.type %in% c("flux_no_estimate", "stock_no_estimate")){
      ...
      text.title <-  eval(parse(text = paste0("expression(bold(", X.draw$variable.name, ifelse(failure_C_closure, paste0('*"*"'), ""), "))")))
      text.main <- NULL
